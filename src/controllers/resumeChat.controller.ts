...
export function registerResumeChatHandlers(io: Server): void {
	io.on("connection", (socket: Socket) => {
		...
		socket.on(
			"resume:message",
			async (data: { sessionId: string; content: string; refineMode?: boolean }) => {
				try {
					...
					await resumeChatService.streamResponse(sessionId, content.trim(), callbacks, {
						refineMode,
					});
				} catch (error: any) {
		...
		socket.on("resume:sectionComplete", async (data: { sessionId: string; section: string }) => {
			try {
				...
				await resumeChatService.markSectionComplete(sessionId, section as any);
