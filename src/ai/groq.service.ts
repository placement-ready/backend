...
Generate exactly ${questionCount} questions appropriate for this role and level.`;
		...
		try {
			...
			const parsed = this.parseJsonResponse<{ questions: GeneratedQuestion[] }>(
...
Provide a comprehensive evaluation of all responses.`;
		...
		try {
			...
			const evaluation = this.parseJsonResponse<InterviewEvaluation>(
				completion.choices[0]?.message?.content || "",
			);
...
Generate an appropriate follow-up question.`;
		...
		try {
			...
			return this.parseJsonResponse<FollowUpResponse>(
				completion.choices[0]?.message?.content || "",
			);
		} catch (error: any) {
